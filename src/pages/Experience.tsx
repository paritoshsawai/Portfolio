    ...
    <div className="py-8">
      ...
      <div className="space-y-6">
        {experiences.map((exp, index) => (
          <div
            key={index}
            className="border border-border rounded-md p-6 bg-sidebar/20 hover:shadow-md transition-all duration-300"
          >
            <div className="flex items-center mb-2">
              <Building className="mr-2 text-accent" size={18} />
              <h2 className="text-xl font-semibold">{exp.title}</h2>
            </div>
            <div className="flex flex-col sm:flex-row sm:items-center text-sm space-y-1 sm:space-y-0 sm:space-x-4 mb-4">
            ...
            <ul className="mt-2 space-y-2">
              {exp.responsibilities.map((resp, respIndex) => (
                <li
                  key={respIndex}
                  className="flex items-start hover:bg-sidebar/30 p-2 rounded-md transition-colors"
                >
